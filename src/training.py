...
from src.load_dataset import to_density_matrix, copy_images_bottom_channel, copy_images_bottom_channel_stride
...
def train_net_stride(batch_size, I, J, network, train_loader, criterion, optimizer, stride, device):
    network.train()  # put in train mode: we will modify the weights of the network
    train_loss = 0  # initialize the loss
    train_accuracy = 0  # initialize the accuracy
...
def test_net_stride(batch_size, I, J, network, test_loader, criterion, device, stride):
...
def train_globally(batch_size, I, J, k, network, reduced_train_loader, reduced_test_loader, optimizer, criterion, train_epochs, test_interval, stride, device):
    loss_list = []
    accuracy_list = []
    for epoch in range(train_epochs):
        start = time.time()
        train_loss, train_accuracy = train_net_stride(batch_size, I, J, network, reduced_train_loader, criterion, optimizer, stride, device)
        ...
        if epoch % test_interval == 0:
            test_loss, test_accuracy = test_net_stride(batch_size, I, J, network, reduced_test_loader, criterion, device, stride)
